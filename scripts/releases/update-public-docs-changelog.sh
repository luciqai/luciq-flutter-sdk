...
# (luciqai/luciq-docs) as a GitBook "update" block in changelog/flutter-sdk.md.
#
# The Flutter SDK publishes to pub.dev (via Escape) rather than mirroring a
...
#
# Transformations applied (private CHANGELOG.md -> docs flutter-sdk.md):
...
SOURCE_CHANGELOG="packages/luciq_flutter/CHANGELOG.md"
ORIG_DIR="$(pwd)"
...
git commit -m "Update Flutter docs changelog for v$SDK_VERSION"
echo "Pushing branch: $PR_BRANCH_NAME"
git push origin "$PR_BRANCH_NAME"

PR_TITLE="Update Flutter docs changelog for v$SDK_VERSION"
PR_DESCRIPTION="## Description of the change
Adds the v$SDK_VERSION release block to \`$DOCS_FILE\`, converted from the Flutter SDK \`CHANGELOG.md\` into the GitBook changelog format.
