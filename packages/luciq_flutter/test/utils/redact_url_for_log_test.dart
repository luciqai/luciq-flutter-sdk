...
void main() {
  group('redactUrlForLog', () {
    ...
    test('returns URL unchanged when no query or fragment', () {
      expect(redactUrlForLog('https://api.example.com/users/123'),
          'https://api.example.com/users/123');
      expect(redactUrlForLog('http://localhost:8081/symbolicate'),
          'http://localhost:8081/symbolicate');
    });
    test('strips simple query and appends marker', () {
      expect(redactUrlForLog('https://api.example.com/users?email=u@x.com'),
    ...
    test('strips trailing question mark', () {
      expect(redactUrlForLog('https://api.example.com/users?'),
          'https://api.example.com/users?<redacted>');
    });
    ...
    test('strips fragment silently when no query present', () {
      expect(redactUrlForLog('https://app.example.com/page#section-2'),
    ...
    test('cuts at fragment when fragment comes first', () {
      expect(redactUrlForLog('https://app.example.com/page#frag?fake'),
          'https://app.example.com/page');
    });
    test('strips user:password@ from authority', () {
      expect(redactUrlForLog('https://user:pass@api.example.com/users/123'),
          'https://api.example.com/users/123');
    });
    test('strips username-only userinfo', () {
      expect(redactUrlForLog('https://alice@api.example.com/users'),
    ...
    test('strips userinfo and query together', () {
      expect(redactUrlForLog('https://u:p@api.example.com/users?token=abc'),
          'https://api.example.com/users?<redacted>');
    });
    test('does not strip an @ that appears in the path', () {
      expect(redactUrlForLog('https://api.example.com/users/@me/profile'),
