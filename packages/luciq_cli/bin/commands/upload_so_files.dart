...
/// Usage:  luciq upload-so-files --arch \<arch\> --file \<path\> --api_key \<key\> --token \<token\> --name \<name\>
  ...
  static Future<void> uploadSoFiles(UploadSoFilesOptions options) async {
    try {
      ...
      if (!await file.exists()) {
        stderr.writeln('[Luciq-CLI] Error: File not found: ${options.file}');
      ...
      if (!file.path.endsWith('.zip')) {
        stderr.writeln(
          '[Luciq-CLI] Error: File is not a zip file: ${options.file}',
        );
      ...
      if (!validArchs.contains(options.arch)) {
        stderr.writeln(
          '[Luciq-CLI] Error: Invalid architecture: ${options.arch}. Valid options: ${validArchs.join(', ')}',
        );
        throw Exception(
      ...
      if (response.statusCode < 200 || response.statusCode >= 300) {
        ...
        stderr.writeln('[Luciq-CLI] Error: Failed to upload .so files');
      ...
      stderr.writeln('[Luciq-CLI] Error uploading .so files, $e');
      stderr.writeln('[Luciq-CLI] Error Stack Trace: ${StackTrace.current}');
      exit(1);
    }
  }
}
