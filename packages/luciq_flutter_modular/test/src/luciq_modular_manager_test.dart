...
import 'package:luciq_flutter_modular/src/luciq_modular_manager.dart';
import 'package:mockito/annotations.dart';
import 'package:modular_core/modular_core.dart';

import 'luciq_modular_manager_test.mocks.dart';

@GenerateNiceMocks([
  MockSpec<Module>(),
  MockSpec<LuciqModule>(),
...
void main() {
  late MockLuciqModule mockLuciqModule;
  ...
  setUp(() {
    mockLuciqModule = MockLuciqModule();
  ...
  test('[wrapRoutes] with simple list', () {
    ...
    // Act
    final wrappedRoutes = LuciqModularManager.instance.wrapRoutes(routes);
  ...
  test('[wrapRoute] with [ModuleRoute] should wrap with [LuciqModule]', () {
    // Arrange
    final route = ModuleRoute('/home', module: mockLuciqModule);

    // Act
    final wrappedRoute =
        LuciqModularManager.instance.wrapRoute(route) as dynamic;

    // Assert
    expect(wrappedRoute.name, '/home');
    expect(wrappedRoute.context.module, isA<LuciqModule>());
  });

  test(
      '[wrapRoute] with [ParallelRoute] should wrap child with [LuciqCaptureScreenLoading]',
      () {
    // Arrange
    ...
    expect(widget, isA<LuciqCaptureScreenLoading>());
  });

  test(
      '[wrapRoutes] should wrap child property with [LuciqCaptureScreenLoading] and preserve the rest of its properties',
      () {
    // Arrange
    ...
    // Act
    final wrappedRoutes =
        LuciqModularManager.instance.wrapRoutes(routes) as List<dynamic>;
  ...
  test('[wrapRoute] with custom parent path should add it to the path', () {
    ...
    // Act
    final wrappedRoute = LuciqModularManager.instance
