  ...
  }
  {
    FlutterBasicMessageChannel *channel =
      [[FlutterBasicMessageChannel alloc]
        ...
        codec:LuciqHostApiGetCodec()];
    if (api) {
      NSCAssert([api respondsToSelector:@selector(logUserEventName:parameters:error:)], @"LuciqHostApi api (%@) doesn't respond to @selector(logUserEventName:parameters:error:)", api);
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        NSArray *args = message;
        NSString *arg_name = GetNullableObjectAtIndex(args, 0);
        NSDictionary<NSString *, NSString *> *arg_parameters = GetNullableObjectAtIndex(args, 1);
        FlutterError *error;
        [api logUserEventName:arg_name parameters:arg_parameters error:&error];
        callback(wrapResult(nil, error));
      }];
    } else {
      [channel setMessageHandler:nil];
