import 'package:flutter/widgets.dart';
import 'package:flutter_test/flutter_test.dart';
import 'package:luciq_flutter/luciq_flutter.dart';
import 'package:luciq_flutter/src/generated/luciq_log.api.g.dart';
import 'package:mockito/annotations.dart';
import 'package:mockito/mockito.dart';

import 'luciq_log_test.mocks.dart';

@GenerateMocks([
  LuciqLogHostApi,
])
void main() {
  TestWidgetsFlutterBinding.ensureInitialized();
  WidgetsFlutterBinding.ensureInitialized();

  final mHost = MockLuciqLogHostApi();

  setUpAll(() {
    LuciqLog.$setHostApi(mHost);
  });

  test('[logVerbose] should call host method', () async {
    const message = "Log message";

    await LuciqLog.logVerbose(message);

    verify(
      mHost.logVerbose(message),
    ).called(1);
  });

  test('[logDebug] should call host method', () async {
    const message = "Log message";

    await LuciqLog.logDebug(message);

    verify(
      mHost.logDebug(message),
    ).called(1);
  });

  test('[logInfo] should call host method', () async {
    const message = "Log message";

    await LuciqLog.logInfo(message);

    verify(
      mHost.logInfo(message),
    ).called(1);
  });

  test('[logWarn] should call host method', () async {
    const message = "Log message";

    await LuciqLog.logWarn(message);

    verify(
      mHost.logWarn(message),
    ).called(1);
  });

  test('[logError] should call host method', () async {
    const message = "Log message";

    await LuciqLog.logError(message);

    verify(
      mHost.logError(message),
    ).called(1);
  });

  test('[clearAllLogs] should call host method', () async {
    await LuciqLog.clearAllLogs();

    verify(
      mHost.clearAllLogs(),
    ).called(1);
  });
}
