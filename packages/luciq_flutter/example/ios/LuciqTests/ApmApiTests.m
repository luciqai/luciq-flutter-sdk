...
- (void)setUp {
    self.mAPM = OCMClassMock([LCQAPM class]);
...
- (void)testEndScreenRenderForAutoUiTrace {
    ...
    OCMVerify([self.mAPM endAutoUITraceCPWithFrames:[OCMArg checkWithBlock:^BOOL(NSArray<LCQFrameInfo *> *frames) {
        // Verify that we have the correct number of frames
        XCTAssertEqual(frames.count, 3);
        
        // Verify the first frame
        LCQFrameInfo *firstFrame = frames[0];
        ...
        LCQFrameInfo *secondFrame = frames[1];
        ...
        LCQFrameInfo *thirdFrame = frames[2];
...
- (void)testEndScreenRenderForCustomUiTrace {
    ...
    OCMVerify([self.mAPM endCustomUITraceCPWithFrames:[OCMArg checkWithBlock:^BOOL(NSArray<LCQFrameInfo *> *frames) {
        // Verify that we have the correct number of frames
